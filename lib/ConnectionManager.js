...
class ConnectionManager {
    ...
    _hasRoomInSocketServer(roomName) {
        return this.socketIoServer.of(DEFAULT_NAMESPACE).adapter.rooms.get(roomName);
    }
     ...
     _startServer() {
        ...
        this.socketIoServer.engine.on('initial_headers', (headers, req) => {
            let origin = '*';
        ...
        this.socketIoServer.engine.on('headers', (headers, req) => {
            let origin = '*';
            if (req.headers.origin !== undefined) {
                origin = req.headers.origin;
                headers['Access-Control-Allow-Credentials'] = 'true';
