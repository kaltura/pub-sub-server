...
class ConnectionManager {
    ...
    _validateClient(partnerId, token, socket) {
        ...
        try {
            ...
            if (tokens.key !== KalturaConfig.config.tokens.key) {
                this._connectionError('key', KalturaConfig.config.tokens.key, tokens.key);
                return false;
            }
            ...
            for (let i = 0; i < privileges.length; i++) {
                if ( privileges[i].startsWith(ipRestrict))
                {
                    ...
                    if (ipRestrictionPrivilege[1] !== clientIp) {
                        this._connectionError('client IP', ipRestrictionPrivilege[1], clientIp);
                        return false;
                    }
                }
            }
            ...
            if (tokens.ksExpiry) {
                ...
                if (ksExpiry < now) {
                    this._connectionError('Ks expiry', ksExpiry, now);
                    return false;
                }
            }
	    KalturaLogger.debug("Setting hash for new socket to: " + tokens.hash);
    ...
    _onClientListens(socket, eventNameStr, queueKeyStr) {
        ...
        try {
            ...
            this.socketIoServer.to(socket.id).emit('connected', queueKey, queueKeyStr);
    ...
    _parseKey(token, socket) {
        try {
            ...
            if (tokens[1] !== socket.myHash) {
                this._connectionError('hash', socket.myHash, tokens[1]);
                throw new Error(KalturaPushServerErrors.PARSE_KEY_ERROR);
            }
            return tokens[0];
        }
        catch (err) {
		KalturaLogger.error(KalturaPushServerErrors.TOKEN_DECRYPTION_ERROR + " : " + util.inspect(token) + " - " +  util.inspect(err));
