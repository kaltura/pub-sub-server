...
KalturaLogger = {
	...
	init: function(){
        ...
        if(KalturaLogger.config.monitorLogName){
            KalturaLogger.monitorFile = fs.openSync(KalturaLogger.config.logDir + '/' + KalturaLogger.config.monitorLogName, 'a');
        }
	},
	...
	notifyLogsRotate: function(){
        ...
        if(KalturaLogger.config.monitorLogName){
            var newMonitorLogHandler = fs.openSync(KalturaLogger.config.logDir + '/' + KalturaLogger.config.monitorLogName, 'a');
            var oldMonitorLogHandler = KalturaLogger.monitorFile;
            KalturaLogger.monitorFile = newMonitorLogHandler;
            fs.closeSync(oldMonitorLogHandler);
        }
	},
    ...
    writeMonitor: function(str){
        if(KalturaLogger.monitorFile)
        {
            fs.writeSync(KalturaLogger.monitorFile, str + '\n');
        }
        else
        {
            console.log(str);
    ...
    monitor: function(str, stackSource){
        KalturaLogger.writeMonitor(KalturaLogger.prefix(stackSource) + ' INFO: ' + str);
    },
