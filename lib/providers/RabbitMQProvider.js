...
RabbitMQProvider.prototype.validateConfig = function(config) {
    ...
    if(!config.hosts || !config.connectionRetries || !config.username || !config.password || !config.timeout)
        ConnectionManager.increaseErrorCounter('MISSING_CONFIGURATION_PARAMETER');
...
RabbitMQProvider.prototype.init = function (config) {
    ...
    if (!config.heartbeat)
        ...
        this.heartbeat = parseInt(config.heartbeat,10);
};
...
RabbitMQProvider.prototype.open = function (callback) {
    ...
    this.connection.on('heartbeat', function() {
        KalturaLogger.log("RabbitMq Heartbeat");
    });
